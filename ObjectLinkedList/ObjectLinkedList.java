...
import IntLinkedList.IntLinkedList;
...
public class ObjectLinkedList {
    ...
    public ObjectLinkedList(Object object) { //Will create one head and a Node.
        head = new Node(); //Creates head Node AKA first node in list. [ ]
        head.object = object; //CHANGED TO 0 //
        head.link = null; //Link head with null bc it is the first node that stores data for the ext node in list.
    }
    
    public boolean insertItem(Object object) { //Adds a node in between head node and first node in front of it.
        ...
        n.object = object; //Make n Node value = item
    ...
    public void printList() { //Prints out the whole list from last to first (no head, to the end with the head)
        ...
        while(z!=null) { //while the list does not have the null address, do the following.
            System.out.println(z.object.toString()); //Print out the value of the current node's data.
    ...
    public boolean deleteItem(String name) { //Deletes a node by cutting the links and letting garbage collector delete it.
        if (name.equals(head.object.getName())) { //If item is found in head, just make the head jump to the next node.
            head.link = head.link.link; //head = to the next Node with all values.
            ...
            while((y != null) || !(y.object.getName().equals(name))) { //Keep going until...
                x = y;
                y = y.link;
            ...
            if (y != null) { //If Item is found...
                ...
                System.out.println("Object is not in list. No deletions made.");
                return false;
            }
        }
    }
    ...
    class Node {
        private Object object; //Stored value
